  ...
  for (i = 0; i <= str.length; i++){
    if (str[i]=== " " || str[i] === str.length){
      ...
      for (var j = palabra.length - 1; j >= 0; j--){
        palInvertida = palInvertida + palabra[j];
      
      palabra = "";
      if (i === str.length){
        strInvertido = strInvertido + palInvertida;
      }else {
        strInvertido = strInvertido + palInvertida + " ";
      }
    }
...
function capicua(numero){
  ...
  var arrayNum = strin.split("");
  var inv = arrayNum.reverse();

  if (string === inv.join("")) retrun "Es capicua";
...
function sortArray(arr) {
  ...
  //Escribe tu código aquí
  /* 
  var orderArr = arr.sort(function (el, proxEl){
    return el.length - proxEl.length
  })
  return orderArr;
  */
